...
export function Info(props: ProductInfoProps) {
  return (
    <div
      className="flex flex-col items-center justify-between px-4 py-3 text-center lg:flex-row lg:text-left"
      {...props}
    ></div>
  )
}
...
export function Ref({ productRef, productColor }: ProductRefProps) {
  return (
    <div className="flex flex-col py-1.5">
      <span className="text-md font-medium text-zinc-900">{productRef}</span>
      <span className="text-sm text-zinc-500">{productColor}</span>
    </div>
  )
}
